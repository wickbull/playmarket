...
class FreeKassaPayment
{
     ...
     */
    function __construct($merchant_id, $secret_word, $secret_word2 = null)
    {
        $this->merchant_id = $merchant_id;
        $this->secret_word = $secret_word;
        $this->secret_word2 = $secret_word2;
        $this->url = 'http://www.free-kassa.ru/';
    }
    ...
    {
        if (!is_numeric($order_amount)) throw new \Exception("Сума платежа должна быть числом");
        if (!is_numeric($order_id)) throw new \Exception("Номер заказа должна быть числом");
    ...
    {
        if (empty($this->secret_word2)) throw new \Exception("Укажите Секретное слово 2");
        if (!is_numeric($order_amount)) throw new \Exception("Сума платежа должна быть числом");
        if (!is_numeric($order_id)) throw new \Exception("Номер заказа должна быть числом");
